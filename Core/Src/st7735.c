...
void ST7735_Draw(uint16_t x, uint16_t y, uint16_t color, uint16_t bgColor, const FontMeDef *pix)
{
  uint32_t i, j;
  uint8_t temp;
  uint16_t count = 0;
  uint16_t len = 0;
  //绘制范围
  ST7735_SetAddressWindow(x, y, x + pix->width - 1, y + pix->height - 1);
  ST7735_WriteCommand(0x2C);

  len = pix->height * pix->width;
  int des = len / 8;
  temp = pix->data[count];
  for(i = 0; i < des ; i++ ) {
    for(j = 0; j < 8 ; j++ ) {
...
void ST7735_Clean(uint16_t x, uint16_t y, uint16_t color, uint16_t bgColor,uint16_t width,uint16_t height){
  uint32_t i, j;
  uint8_t temp;
  uint16_t len = 0;
  //绘制范围
...
void ST7735_DrawTime(uint16_t x, uint16_t y, char c, uint16_t color, uint16_t bgColor, const FontMeDef *font)
{
  uint32_t i, j;
  uint8_t b;
  uint32_t temp;
  uint8_t ji = (font->width * font->height) / 8;
  ST7735_SetAddressWindow(x, y, x + font->width - 1, y + font->height - 1);
  ST7735_WriteCommand(0x2C);
 
  if(c == ' ')      temp = 0;
  else if(c == ':') temp = 11 * ji;
  else              temp = ((c - '0') + 1) * ji;
  b = font->data[temp];
  for(i = 0; i < ji ; i++ ) {
    for(j = 0; j < 8 ; j++ ) {
...
void ST7735_DrawStringMe(uint16_t x, uint16_t y, const char *str, uint16_t color, uint16_t bgColor, const FontMeDef *font)
{
  while (*str) {
    ...
    ST7735_DrawTime(x, y, *str, color, bgColor, font);
    x += font->width;
    str++;
  }
}
